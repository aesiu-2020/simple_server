...
class PersonTest(LiveServerTestCase):
	...
	def test_full_CRUD(self):
		...
		response = requests.post(self.get_server_url() + "/rest/person", json = basic_request)
		...
		response = requests.put(self.get_server_url() + "/rest/person/" + uuid, json = basic_request)
		response_body = response.json()
		self.assertEqual(response.status_code, 200)
		self.assertEqual(response_body['firstName'], "Homer")
		...
		## and delete
		response = requests.delete(self.get_server_url() + "/rest/person/" + uuid)
		...
		# but history remains
		response = requests.get(self.get_server_url() + "/rest/person/" + uuid + "/version/0")
		response_body = response.json()
		self.assertEqual(response.status_code, 200)
		self.assertEqual(response_body['first_name'], "Homer")
		self.assertEqual(response_body['middle_name'], None)

		response = requests.get(self.get_server_url() + "/rest/person/" + uuid + "/version/1")
		response_body = response.json()
		self.assertEqual(response.status_code, 200)
		self.assertEqual(response_body['first_name'], "Homer")
		self.assertEqual(response_body['middle_name'], "Jay")

		# last entry exists to say that this person has no data anymore/was deleted
		response = requests.get(self.get_server_url() + "/rest/person/" + uuid + "/version/2")
		response_body = response.json()
		self.assertEqual(response.status_code, 200)
	...
	def test_bad_fetches(self):
		...
		response = requests.get(self.get_server_url() + "/rest/person/" + str(uuid4()))
		self.assertEqual(response.status_code, 404)
		response = requests.delete(self.get_server_url() + "/rest/person/" + str(uuid4()))
		self.assertEqual(response.status_code, 404)
		response = requests.put(self.get_server_url() + "/rest/person/" + str(uuid4()))
		self.assertEqual(response.status_code, 404)
	...
	def test_failed_writes(self):
		...
		response = requests.post(self.get_server_url() + "/rest/person", json = request)
		# no name, no go
		self.assertEqual(response.status_code, 400)
